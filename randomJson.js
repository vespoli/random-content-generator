...
randomJson = {
	...
	parseVars: function (a, c) {
		...
		else "object" == typeof a ? a = this.parseObject(a, 0) : (b.test(a) && (a = a.replace(b, rndFirst[this.random])), d.test(a) && (a = a.replace(d, rndLast[this.random])), e.test(a) && (a = a.replace(e, rndCompanies[this.random])), h.test(a) && (a = a.replace(h, "+7095" + Math.random().toFixed(7).split(".")[1])), j.test(a) && (a = a.replace(j, (this.transliterate(rndFirst[this.random]) + "@" + rndCompanies[this.random].replace(/[^a-zA-Z0-9]+/g,'') + ".com").toLowerCase())), l.test(a) && (a = Math.floor(2 * Math.random()) ? !0 : !1), k.test(a) && (a = m.test(a) ? c : a.replace(k, c)), i.test(a) && (g = i.exec(a), g = Math.round(g[1] - 0.5 + Math.random() * (g[2] - g[1] + 1)), a = f.test(a) ? g : a.replace(i, g)), n.test(a) && (f = new Date, f.setTime(f.getTime() * Math.random()), a = f.toUTCString()));
		return a;
	},
